...
def obtener_segundos_actuales(p_estado):
    """Calcula los segundos transcurridos basándose en marcas de tiempo UTC absolutas"""
...
def main(page: ft.Page):
    ...
    def cargar_partidos_bd():
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT id, fecha, equipo_rival, tiempos_por_partido, minutos_por_tiempo, jugadores_en_cancha, goles_local, goles_rival, segundos, titulares, eventos, minutos_partido, finalizado, segundos_acumulados, hora_inicio FROM partidos ORDER BY fecha DESC, id DESC"
        )
        filas = cursor.fetchall()
        conn.close()
        ...
        lista_partidos = []
        for row in filas:
            seg_acum = row[13] if len(row) > 13 and row[13] is not None else row[8]
            h_inicio = row[14] if len(row) > 14 else None
        ...
        if not lista_partidos:
            crear_partido_bd(datetime.now().strftime("%Y-%m-%d"), "Rival FC", 2, 10, 7)
            cargar_partidos_bd()
    ...
    def crear_partido_bd(fecha, rival, tiempos, minutos_tiempo, jugadores_cancha):
        ...
        cursor.execute(
            """
            INSERT INTO partidos (fecha, equipo_rival, tiempos_por_partido, minutos_por_tiempo, jugadores_en_cancha, segundos_acumulados, hora_inicio, updated_at)
    ...
    def sincronizar_partido_activo_bd():
        """Obtiene el estado más reciente desde Turso para mantener en sincronía varios dispositivos"""
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "SELECT goles_local, goles_rival, segundos, segundos_acumulados, hora_inicio, titulares, eventos, minutos_partido, finalizado FROM partidos WHERE id=?",
                (estado["partido_activo_id"],),
            )
            row = cursor.fetchone()
            conn.close()
            if row:
                estado["goles_local"] = row[0]
                estado["goles_rival"] = row[1]
                estado["segundos_acumulados"] = row[3] if row[3] is not None else row[2]
                estado["hora_inicio"] = row[4]
                estado["corriendo"] = estado["hora_inicio"] is not None
                estado["titulares_seleccionados"] = json.loads(row[5])
                estado["eventos_registrados"] = json.loads(row[6])
                estado["minutos_partido_actual"] = json.loads(row[7])
                estado["finalizado"] = bool(row[8])
        except Exception:
            pass
    ...
    def guardar_estado_partido_activo():
        ...
        cursor.execute(
            """
            UPDATE partidos
            SET goles_local=?, goles_rival=?, segundos=?, segundos_acumulados=?, hora_inicio=?, titulares=?, eventos=?, minutos_partido=?, finalizado=?, updated_at=DATETIME('now')
            WHERE id=?
        """,
            (
                estado["goles_local"],
        ...
        while True:
            ...
            if estado["corriendo"] and not estado["finalizado"]:
                if seg >= duracion_total_partido:
                    ...
                    texto_alerta_cambio.color = COLOR_ROJO
                    guardar_estado_partido_activo()
                elif seg > 0 and seg % duracion_tiempo_segs == 0 and seg < duracion_total_partido:
    ...
    def view_configuracion():
        ...
        tf_minutos_tiempo = ft.TextField(
            label="Min/Tiempo",
            value="10",
            keyboard_type=ft.KeyboardType.NUMBER,
            width=100,
        ...
        tf_jugadores_cancha = ft.TextField(
            label="Cancha",
            value="7",
            keyboard_type=ft.KeyboardType.NUMBER,
            width=90,
    ...
    def view_plantel():
        ...
        tf_nuevo_num = ft.TextField(
            label="N°", width=60, border_color=COLOR_BORDE, focused_border_color=COLOR_CELESTE, border_radius=10, disabled=es_bloqueado
        )
        tf_nuevo_nom = ft.TextField(
            label="Nombre Jugador", expand=True, border_color=COLOR_BORDE, focused_border_color=COLOR_CELESTE, border_radius=10, disabled=es_bloqueado
        )
        tf_nuevo_puesto = ft.TextField(
            label="Puesto", width=110, border_color=COLOR_BORDE, focused_border_color=COLOR_CELESTE, border_radius=10, disabled=es_bloqueado
        )
        ...
        alerta_bloqueo = (
            ft.Container(
                content=ft.Text(
                    "🔒 PARTIDO FINALIZADO - Para modificar la alineación debes reabrir el partido.",
                    color=COLOR_AMBAR,
                    size=11,
                ...
                bgcolor=COLOR_TARJETA,
                border_radius=8,
                border=ft.border.all(1, COLOR_AMBAR),
            )
            if es_bloqueado
            else ft.Container()
        )
    ...
    def view_partido():
        ...
        def detener_reloj(e):
            ...
            def cerrar_dlg(ev):
                dialogo_reset_reloj.open = False
                page.update()
            ...
            def procesar_reset(ev):
                ...
                texto_alerta_cambio.value = "⏹️ Partido e historial de eventos reiniciados"
                texto_alerta_cambio.color = COLOR_SUBTEXTO
            ...
            dialogo_reset_reloj = ft.AlertDialog(
                title=ft.Text("¿Reiniciar Partido y Cronómetro?", color=COLOR_TEXTO),
                content=ft.Text(
                    "Esta acción pondrá el reloj en 00:00, reiniciará el marcador a 0-0 y eliminará TODOS los eventos registrados en este partido.",
                    color=COLOR_SUBTEXTO,
                ),
                actions=[
                    ft.TextButton("Cancelar", on_click=cerrar_dlg),
                    ft.ElevatedButton("Sí, Reiniciar Todo", bgcolor=COLOR_ROJO, color=COLOR_TEXTO, on_click=procesar_reset),
                ],
                bgcolor=COLOR_TARJETA,
            )
            page.overlay.append(dialogo_reset_reloj)
            dialogo_reset_reloj.open = True
            page.update()
                ...
                ft.Row(
                    [
                        ...
                        ft.IconButton(
                            ...
                            icon_size=36,
                            icon_color=COLOR_ROJO if not es_fin else COLOR_SUBTEXTO,
                            bgcolor=COLOR_BORDE,
                            disabled=es_fin,
                            on_click=detener_reloj,
                        ),
                    ],
                    alignment=ft.MainAxisAlignment.CENTER,
                ),
    ...
    def view_resumen():
        ...
        def view_resumen_contenido(fecha=""):
            if fecha:
                partidos_base = [p for p in estado["partidos"] if p["fecha"] == fecha]
                titulo = f"📊 Tabla de Posiciones ({fecha})"
            else:
                partidos_base = estado["partidos"]
                titulo = "📊 Tabla de Posiciones (Histórico Total)"

            partidos_mostrar = [
                p
                for p in partidos_base
                if p.get("segundos", 0) > 0 or len(p.get("eventos", [])) > 0 or p.get("finalizado", False)
            ]
                ...
                return ft.Container(
                    content=ft.Column(
                        [
                            ...
                            ft.Text("No hay partidos jugados o en curso para mostrar", color=COLOR_SUBTEXTO, size=14),
                        ],
                        horizontal_alignment=ft.CrossAxisAlignment.CENTER,
                        spacing=6,
                    ),
            ...
            for p in partidos_mostrar:
                nombre_rival_partido = p.get("equipo_rival", "").strip().lower()
                for ev in p.get("eventos", []):
                    if ev.get("evento") == "Gol":
                        txt_jugador = ev.get("jugador", "").strip()
                        txt_lower = txt_jugador.lower()

                        if "equipo rival" in txt_lower or "rival" in txt_lower or (
                            nombre_rival_partido and nombre_rival_partido in txt_lower
                        ):
                            continue

                        jugador_limpio = txt_jugador.replace("Gol de ", "").strip()
                        if jugador_limpio:
                            goleadores[jugador_limpio] = goleadores.get(jugador_limpio, 0) + 1
                ...
                rows=[
                    ft.DataRow(
                        cells=[
                            ...
                            ft.DataCell(content=ft.Text("Dunalastair", weight=ft.FontWeight.BOLD, color=COLOR_TEXTO)),
            ...
            for i, p in enumerate(partidos_mostrar, 1):
                ...
                if eventos_goles:
                    resumen_evs.append(
                        ft.Text(
                            "⚽ Goles: " + ", ".join([f"{g['jugador']} ({g['minuto']})" for g in eventos_goles]),
                            size=11,
                            color=COLOR_SUBTEXTO,
                        )
                    )
                if eventos_tarjetas:
                    resumen_evs.append(
                        ft.Text(
                            "🟨/🟥 Tarjetas: " + ", ".join([f"{t['jugador']} ({t['evento']})" for t in eventos_tarjetas]),
                            size=11,
                            color=COLOR_SUBTEXTO,
                        )
                    )
                ...
                card = ft.Container(
                    content=ft.Column(
                        [
                            ...
                            ft.Text(
                                f"Duración: {p['segundos'] // 60} min jugados | Titulares: {len(p['titulares'])}",
                                size=11,
                                color=COLOR_SUBTEXTO,
                            ),
        ...
        contenedor_resumen_datos = ft.Container(content=view_resumen_contenido(fecha_filtro))

        def al_cambiar_fecha(e):
            contenedor_resumen_datos.content = view_resumen_contenido(tf_fecha_filtro.value.strip())
            page.update()
    ...
    header_app = ft.Container(
        content=ft.Row(
            [
                ...
                ft.Container(
                    content=ft.Text("Turso Sync BD", size=10, weight=ft.FontWeight.BOLD, color=COLOR_CELESTE),
                    bgcolor=COLOR_BORDE,
