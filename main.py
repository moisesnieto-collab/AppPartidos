...
def main(page: ft.Page):
    ...
    def view_configuracion():
        ...
        def click_generar_grupo(e):
            ...
            if not nom_g or not eq_princ or not rivales_str:
                texto_feedback_grupo.value = (
                    "⚠️ Completa todos los campos del grupo."
                )
            ...
            lista_rivales = [
                r.strip() for r in rivales_str.split(",") if r.strip()
            ]
            if not lista_rivales:
                texto_feedback_grupo.value = (
                    "⚠️ Ingresa al menos un equipo rival."
                )
                texto_feedback_grupo.color = COLOR_ROJO
                page.update()
                return
        ...
        def confirmar_reset(e):
            ...
            dialogo_reset = ft.AlertDialog(
                ...
                content=ft.Text(
                    "Se eliminarán todos los grupos, partidos y estadísticas."
                ),
                actions=[
        ...
        if estado["grupo_activo"] and estado["partidos_grupo"]:
            ...
            for pos, (nombre_eq, st) in enumerate(equipos_ordenados, start=1):
                es_mi_equipo = (
                    nombre_eq.lower() == grupo["equipo_principal"].lower()
                )
                color_nombre = COLOR_CELESTE if es_mi_equipo else COLOR_TEXTO
                peso_nombre = (
                    ft.FontWeight.BOLD if es_mi_equipo else ft.FontWeight.NORMAL
                )
                ...
                filas_tabla.append(
                    ft.DataRow(
                        cells=[
                            ft.DataCell(
                                content=ft.Text(str(pos), color=COLOR_TEXTO)
                            ),
                            ft.DataCell(
                                content=ft.Text(
                                    nombre_eq,
                            ...
                            ft.DataCell(
                                content=ft.Text(str(st["PJ"]), color=COLOR_TEXTO)
                            ),
                            ft.DataCell(
                                content=ft.Text(str(st["PG"]), color=COLOR_TEXTO)
                            ),
                            ft.DataCell(
                                content=ft.Text(str(st["PE"]), color=COLOR_TEXTO)
                            ),
                            ft.DataCell(
                                content=ft.Text(str(st["PP"]), color=COLOR_TEXTO)
                            ),
                            ft.DataCell(
                                content=ft.Text(str(st["GF"]), color=COLOR_TEXTO)
                            ),
                            ft.DataCell(
                                content=ft.Text(str(st["GC"]), color=COLOR_TEXTO)
                            ),
                            ft.DataCell(
                                content=ft.Text(
                                    f"{st['DG']:+d}", color=COLOR_TEXTO
                                )
                            ),
                            ft.DataCell(
                                content=ft.Text(
            ...
            tabla_ui = ft.DataTable(
                columns=[
                    ...
                    ft.DataColumn(
                        label=ft.Text(
                            "PG", weight=ft.FontWeight.BOLD, color=COLOR_VERDE
                        )
                    ),
                    ft.DataColumn(
                        label=ft.Text(
                            "PE", weight=ft.FontWeight.BOLD, color=COLOR_AMBAR
                        )
                    ),
                    ft.DataColumn(
                        label=ft.Text(
                            "PP", weight=ft.FontWeight.BOLD, color=COLOR_ROJO
                        )
                    ),
                    ft.DataColumn(
                        label=ft.Text(
                    ...
                    ft.DataColumn(
                        label=ft.Text(
                            "Pts", weight=ft.FontWeight.BOLD, color=COLOR_VERDE
                        )
                    ),
                ],
                rows=filas_tabla,
                ...
                if p["es_principal"]:
                    ...
                    card = ft.Container(
                        ...
                        border=ft.border.all(
                            1.5, COLOR_CELESTE if es_activo else COLOR_BORDE
                        ),
                    )
        ...
        return ft.Column(
            [
                ...
                ft.Text(
                    f"2. Partidos de {estado['config']['equipo_principal']}",
                    weight=ft.FontWeight.BOLD,
                    color=COLOR_CELESTE,
                ),
                ft.Column(
                    ...
                    or [
                        ft.Text(
                            "Sin partidos asignados", color=COLOR_SUBTEXTO
                        )
                    ],
                    spacing=8,
                ),
                ft.Text(
                    "3. Marcadores entre Rivales del Grupo (Combinatoria)",
                    weight=ft.FontWeight.BOLD,
                    color=COLOR_AMBAR,
                ),
                ft.Column(
                    ...
                    or [
                        ft.Text(
                            "Sin partidos de rivales", color=COLOR_SUBTEXTO
                        )
                    ],
                    spacing=6,
                ),
    ...
    def view_plantel():
        ...
        return ft.Column(
            [
                ...
                ft.Container(
                    content=ft.Column(
                        controls=titulares_ui
                        or [ft.Text("Sin titulares", color=COLOR_SUBTEXTO)]
                    ),
                    bgcolor=COLOR_TARJETA,
                    padding=8,
                    border_radius=12,
                ...
                ft.Container(
                    content=ft.Column(
                        controls=suplentes_ui
                        or [ft.Text("Sin suplentes", color=COLOR_SUBTEXTO)]
                    ),
                    bgcolor=COLOR_TARJETA,
                    padding=8,
                    border_radius=12,
    ...
    def view_partido():
        ...
        fecha_partido = estado["config"].get(
            "fecha", datetime.now().strftime("%Y-%m-%d")
        )
        ...
        marcador_ui = ft.Container(
            content=ft.Column(
                [
                    ft.Text(
                        f"📅 {fecha_partido}", size=11, color=COLOR_SUBTEXTO
                    ),
                    ft.Row(
                        [
                            ft.Column(
        ...
        def registrar_evento_click(e):
            ...
            if tipo_evento == "Gol":
                ...
                if not jugador_entra:
                    texto_status_evento.value = (
                        "⚠️ Selecciona suplente que entra."
                    )
                    texto_status_evento.color = COLOR_ROJO
                    page.update()
                    return
        ...
        return ft.Column(
            [
                    ...
                    [
                        ...
                        ft.OutlinedButton(
                            ...
                            style=ft.ButtonStyle(
                                ...
                                side=ft.BorderSide(
                                    1, COLOR_VERDE if es_fin else COLOR_ROJO
                                ),
                            ),
                ...
                marcador_ui,
                ft.Container(
                    content=texto_reloj, alignment=ft.Alignment(0, 0)
                ),
                ft.Container(
                    content=texto_alerta_cambio, alignment=ft.Alignment(0, 0)
                ),
                ft.Row(
                    [
                        ft.IconButton(
    ...
    page.navigation_bar = ft.NavigationBar(
        ...
        destinations=[
            ft.NavigationBarDestination(
                icon=ft.Icons.SETTINGS, label="Config & Tabla"
            ),
            ft.NavigationBarDestination(icon=ft.Icons.PEOPLE, label="Plantel"),
            ft.NavigationBarDestination(
                icon=ft.Icons.SPORTS_SOCCER, label="Partido"
            ),
            ft.NavigationBarDestination(
                icon=ft.Icons.BAR_CHART, label="Ranking"
            ),
        ],
    )
        ...
        content=ft.Row(
            [
                ft.Row(
                    [
                        ft.Icon(
                            ft.Icons.SPORTS_SOCCER, color=COLOR_CELESTE, size=22
                        ),
                        ft.Text(
                            "Real Dunalastair FC",
                            weight=ft.FontWeight.BOLD,
